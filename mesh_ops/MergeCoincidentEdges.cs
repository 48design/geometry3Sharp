...
namespace gs
{
	...
	public class MergeCoincidentEdges
	{
		...
		/// <summary>
		/// Determines if edges that are repeated more than one time (e.g. 3 overlapping edges) are ignored
		/// </summary>
			...
			while (Q.Count > 0)
			{
				...
				for (int i = 0; i < equiv.Count && merged == false; ++i)
				{
					...
					if (Mesh.IsEdge(other_eid) == false || Mesh.IsBoundaryEdge(other_eid) == false)
						continue;
					// Util.DebugEdgeInfo(Mesh, e.eid, other_eid);
					...
					{
						// Debug.WriteLine($"merged Ok: {e.eid} with {other_eid}");
